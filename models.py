...
class ConditionalConv(nn.Module):
    ...
    def __init__(
        ...
        self.conditioner = nn.Sequential(nn.Linear(cond_c, n_param), ConditionerActivation())
...
class ConditionalInstanceNorm(nn.Module):
    ...
    def __init__(self, cond_c: int, n_dim: int, in_c: int, affine: bool = True) -> None:
        ...
        **kwargs,
        ...
        for lvl in range(n_down):
            setattr(
                self,
                "down_" + str(lvl),
                conv(2 ** (lvl) * C, 2 ** (lvl + 1) * C, 3, 2, 1),
            )
            setattr(self, "down_act_" + str(lvl), ReLUModule())
            setattr(
                self,
                "down_norm_" + str(lvl),
                norm_layer(2 ** (lvl + 1) * C, affine=True),
            )
            for l in range(n_fix):
                setattr(
                    self,
                    f"conv_{lvl+1}_{l}",
                    conv(2 ** (lvl + 1) * C, 2 ** (lvl + 1) * C, 3, 1, 1),
                )
        ...
        for lvl in range(n_down):
            setattr(
                self,
                "up_" + str(lvl),
                transposed_conv(2 ** (lvl + 1) * C, 2 ** (lvl) * C, 4, 2, 1),
            )
            ...
            for l in range(n_fix):
                if l == 0:
                    setattr(
                        self,
                        "dec_conv_" + str(lvl) + "_0",
                        conv(2 ** (lvl + 1) * C, 2 ** (lvl) * C, 3, 1, 1),
                    )
                else:
                    setattr(
                        self,
                        "dec_conv_" + str(lvl) + "_" + str(l),
                        conv(2 ** (lvl) * C, 2 ** (lvl) * C, 3, 1, 1),
                    )
                setattr(self, "dec_act_" + str(lvl) + "_" + str(l), ReLUModule())
                setattr(
                    self,
                    "dec_norm_" + str(lvl) + "_" + str(l),
                    norm_layer(2 ** (lvl) * C, affine=True),
                )
    ...
    def __init__(
        ...
        for l in range(n_fix):
            ...
            setattr(
                self,
                "norm_0_" + str(l),
                ConditionalInstanceNorm(cond_c, n_dim, C, affine=True),
            )
        for lvl in range(n_down):
            setattr(
                self,
                "down_" + str(lvl),
                ConditionalConv(cond_c, n_dim, 2 ** (lvl) * C, 2 ** (lvl + 1) * C, 3, 2, 1),
            )
            setattr(self, "down_act_" + str(lvl), ReLUModule())
            setattr(
                self,
                "down_norm_" + str(lvl),
                ConditionalInstanceNorm(cond_c, n_dim, 2 ** (lvl + 1) * C, affine=True),
            )
            for l in range(n_fix):
                setattr(
                    self,
                    f"conv_{lvl+1}_{l}",
                    ConditionalConv(cond_c, n_dim, 2 ** (lvl + 1) * C, 2 ** (lvl + 1) * C, 3, 1, 1),
                )
                setattr(self, f"act_{lvl+1}_{l}", ReLUModule())
                setattr(
                    self,
        ...
        for lvl in range(n_down):
            ...
            setattr(
                self,
                "up_norm_" + str(lvl),
                ConditionalInstanceNorm(cond_c, n_dim, 2 ** (lvl) * C, affine=True),
            )
            for l in range(n_fix):
                if l == 0:
                    setattr(
                        self,
                        "dec_conv_" + str(lvl) + "_0",
                        ConditionalConv(cond_c, n_dim, 2 ** (lvl + 1) * C, 2 ** (lvl) * C, 3, 1, 1),
                    )
                else:
                    setattr(
                        self,
                        "dec_conv_" + str(lvl) + "_" + str(l),
                        ConditionalConv(cond_c, n_dim, 2 ** (lvl) * C, 2 ** (lvl) * C, 3, 1, 1),
                    )
                setattr(self, "dec_act_" + str(lvl) + "_" + str(l), ReLUModule())
                setattr(
                    self,
...
class HyperUnet(nn.Module):
    ...
    def __init__(
        self, hypernetwork_layers: list[int], in_c: int, out_c: int, n_down: int, n_fix: int, C: int, n_dim: int = 3
    ) -> None:
        ...
        self.conditioner = nn.Sequential(
            *[
                nn.Sequential(
                    nn.Linear(hypernetwork_layers[i], hypernetwork_layers[i + 1]),
                    nn.ReLU(),
                )
    ...
    def get_unet(self, conditioning: torch.Tensor) -> UNet:
        ...
        if conditioning.size(0) != 1:
            raise ValueError(
                f"Provide only one conditioning variable, size should be (1, {self.hypernetwork_layers[0]})."
            )
        conditioning = self.conditioner(conditioning)
